monthly_predicted_streamflow <- function(total_avg_monthly_predicted){
  ...
  for(i in seq(1,length(monthly_averages))){
    monthly_averages[i] <- mean(discharge_predicted[(j+1):(j+number_of_days_table[i])])
  ...
  total_avg_monthly_predicted <- array(0,12)
  for (i in seq(1,12)){
    total_avg_monthly_predicted[i] <- mean(month_averages_2[i,])
  }
  return(total_avg_monthly_predicted)
}