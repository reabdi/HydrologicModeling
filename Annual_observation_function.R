annual_observed_streamflow <- function(annual_observation){
  ...
  annual_observation <- array(0,length(year_type))
  d3 <- 0
  for (i in seq(1,length(annual_observation))){
    annual_observation[i] <- mean(input[(d3+1):(d3+year_type[i]),2])
    d3 <- d3+year_type[i]
  }
  results <- list(annual_observation,C)
  return(results)
}
